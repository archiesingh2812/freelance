...
skinMenuItem(int id, String name, BuildContext context, Function(int) _onMenuClickItem){
  if (theme.appSkin == "basic") {
    ...
    switch(id){
      ...
      case 3: imageAsset = "assets/addtocart.png"; break;
...
skinHeader(var context, Function(String) callback, String title){
  if (theme.appSkin == "basic")
    return bHeaderWidget(context, callback, Colors.white, title);
  if (theme.appSkin == "smarter")
    return sHeaderWidget(context, callback, Colors.white, title);
}

// only main screen
skinHeaderMenu(var context, Function(String) callback, String title){
  if (theme.appSkin == "basic")
    return bHeaderMenuWidget(context, callback, Colors.white, title);
  if (theme.appSkin == "smarter")
    return sHeaderMenuWidget(context, callback, Colors.white, title);
}

// only product details screen
skinHeaderBackButtonWithBasket(var context, Function(String) callback, Color _color){
  if (theme.appSkin == "basic")
    return bHeaderBackButtonWithBasket(context, callback, Colors.white);
  if (theme.appSkin == "smarter")
    return sHeaderBackButtonWithBasket(context, callback, Colors.white);
}
