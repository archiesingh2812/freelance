...
class OTPScreen extends StatefulWidget {
  ...
  final String photo;
  const OTPScreen(
      {Key key, this.email, this.password, this.name, this.type, this.photo})
      : super(key: key);
  @override
...
class _OTPScreenState extends State<OTPScreen>
  ...
  _onCodeChange(String code) {
    if (code.length == 6)
      _signInWithPhoneNumber(code, (bool ret) {
        if (ret) {
          _waits(true);
          register(widget.email, widget.password, widget.name, widget.type,
              widget.photo, _okUserEnter, _error);
        } else
    ...
    switch (step) {
      case 1:
        if (editControllerPhone.text.isEmpty)
          return openDialog(
              strings.get(27)); // To continue enter your phone number
        _waits(true);
        setState(() {});
  ...
  Future<void> _verifyPhoneNumber(
      _phoneText, Function() callbackok, Function(String) callbackError) async {
    String phone = "";
  ...
  Future<void> _signInWithPhoneNumber(
      String code, Function(bool) callback) async {
    try {
  ...
  _okUserEnter(String name, String password, String avatar, String email,
    ...
    _waits(false);
    account.okUserEnter(name, password, avatar, email, token,
        editControllerPhone.text, 0, typeReg);
    changeProfile(
        token, name, email, editControllerPhone.text, () {}, (String _) {});
  ...
  _body() {
    ...
    if (step == 1) {
      list.add(Container(
        ...
        child: Text(
          strings.get(30), // "Verify phone number"
      ...
      list.add(Container(
          ...
          child: IButton3(
              color: theme.colorPrimary,
              text: strings.get(29),
              textStyle: theme.text14boldWhite, // CONTINUE
              pressButton: () {
                _pressCreateAccountButton();
              })));
    }
    if (step == 2) {
      list.add(Container(
        ...
        child: Text(
          "${strings.get(288)} ${editControllerPhone.text} ${strings.get(289)}", // On phone number send SMS with code. Enter code
          style: theme.text20boldWhite, textAlign: TextAlign.start,
        ),
      ));
      list.add(SizedBox(
      ...
      list.add(
        Container(
          margin: const EdgeInsets.fromLTRB(20, 0, 20, 0),
          child: IVerifySMS(color: Colors.black, callback: _onCodeChange),
        ),
      );
    }
    list.add(SizedBox(
