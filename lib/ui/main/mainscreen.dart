...
class _MainScreenState extends State<MainScreen>
  ...
  void startCityTimer() {
    _timer = new Timer.periodic(
      Duration(seconds: 1),
      (Timer timer) {
        if (homeScreen.mainWindowData == null || _cityScreenOpen) return;
        timer.cancel();
        if (homeScreen.mainWindowData.settings.city.isNotEmpty) {
          var city = pref.get(Pref.city);
          if (city.isNotEmpty) return;
          if (pref.get(Pref.allCity) == "true") return;
          if (city.isEmpty) _cityScreenOpen = true;
          Navigator.push(
            context,
            MaterialPageRoute(
        ...
        child: Scaffold(
          ...
          body: Stack(
            children: <Widget>[
              ...
              if (_currentPage == 1)
                OrdersScreen(
                    onErrorDialogOpen: _openDialogError, onBack: onBack),
              if (_currentPage == 2)
                HomeScreen(
                    onErrorDialogOpen: _openDialogError,
                    redraw: () {
                      setState(() {});
                    },
                    callback: routes,
              ...
              if (_currentPage == 4) FavoritesScreen(scaffoldKey: _scaffoldKey),
              Container(
                color: Colors.white,
                child: skinHeaderMenu(context, onBack, _headerText),
              ),

              if (appSettings.bottomBarType == "type1" &&
                  theme.appSkin == "basic")
                IBottomBar(
                    colorBackground: Color(0xFFfac917),
              ...
              if (appSettings.bottomBarType == "type1" &&
                  theme.appSkin == "smarter")
                IBottomBarSmart(
                    colorBackground: appSettings.bottomBarColor,
              ...
              if (appSettings.bottomBarType == "type2" &&
                ...
                IBottomBar2Smart(
                    colorBackground: appSettings.bottomBarColor,
                    colorSelect: theme.colorPrimary,
                    colorUnSelect: theme.colorDefaultText.withAlpha(100),
                    callback: _bottonBarChange,
                    initialSelect: _currentPage,
                    radius: appSettings.radius,
                    shadow: appSettings.shadow,
              ...
              if (_currentState == "faq") HelpScreen(onBack: onBack),
              // chat
              ...
              if (_currentState == "notify") NotificationScreen(onBack: onBack),
              if (_currentState == "wallet") WalletScreen(onBack: onBack),
              if (_currentState == "order_details")
                OrderDetailsScreen(onBack: onBack),
              if (_currentState == "basket") BasketScreen(onBack: onBack),

              if (wait) skinWait(context, true),
  ...
  onBack(String route) {
    if (route == "open_menu") return _openMenu();
    _currentState = "";
    if (route == "account") _currentPage = 3;
    if (route == "chat") _currentState = "chat";
    if (route == "notify") _currentState = "notify";
    if (route == "basket") {
    ...
    if (route == "order_details") _currentState = "order_details";
    setState(() {});
  }
